...
def _to_sglang_kwargs(
    ...
    *,
    ...
    # (DiffusionNFT reads the clean final latent; GRPO replays the trajectory),
    # so request rollout + the dit-trajectory unconditionally
    # (``rollout_return_dit_trajectory`` gates trajectory collection independently
    # of the SDE kernel).
    kwargs["rollout"] = True
    kwargs["rollout_return_dit_trajectory"] = True
    if sde_indices:
        # GRPO: per-step SDE noise on the resolved step indices.
        # ``num_sde_steps=0`` resolves to ``[]`` (not ``None``) via
        # ``resolve_sde_indices``, so gate on non-emptiness, not ``is not None``.
        require(
